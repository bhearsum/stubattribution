...
type BLKXTable struct {
	...
	Reserved4                 uint32
	Reserved5                 uint32
	Reserved6                 uint32 // 62 bytes to here
	Checksum                  UDIFChecksum // 136 for this, so 198 total now
	BlocksRunCount            uint32 // 202 total now
}
...
func ParseBlkxData(data []uint8) (*BLKXContainer, error) {
	...
	if err := binary.Read(byteReader, binary.BigEndian, table); err != nil {
		return container, fmt.Errorf("blkx: %w", err)
	}
