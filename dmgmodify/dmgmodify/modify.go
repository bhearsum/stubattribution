...
// Note: We explicitly do _not_ update the Attribution resource here, as it
...
func WriteAttributionCode(dmg *dmglib.DMG, sentinel string, code []byte) error {
	...
	blkxRes, _ := dmg.Resources.GetResourceDataByName("blkx")
	...
	// metadata.
	plstRes, _ := dmg.Resources.GetResourceDataByName("plst")
	...
	paddingOffset := fullAttrOffset + len(sentinel)
	for {
	...
	copy(dmg.Data[fullAttrOffset:fullAttrOffset+len(code)], code[:])
	...
	newBlkxChecksum := crc32combine.CRC32Combine(
		0xedb88320,
		crc32combine.CRC32Combine(0xedb88320,
